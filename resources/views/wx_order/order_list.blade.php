...
<div class="mui-segmented-control mui-segmented-control-inverted mui-segmented-control-primary b-white">
    <a class="mui-control-item  font-12 @if(empty($state)) mui-active @endif" href="/order/index">全部</a>
    <a class="mui-control-item font-12 color-80 @if($state == '1') mui-active @endif" href="/order/index/1">待付款</a>
    <a class="mui-control-item font-12 color-80 @if($state == '2') mui-active @endif" href="/order/index/2">待发货</a>
    <a class="mui-control-item font-12 color-80 @if($state == '3') mui-active @endif" href="/order/index/3">待收货</a>
    <a class="mui-control-item font-12 color-80 @if($state == '9') mui-active @endif" href="/order/index/9">已完成</a>
</div>
                ...
                <div class="order-list-count b-white">
                    <ul class="font-12 color-80 flex-end">
                        <li>
                            ...
                            实付款：¥<span class="color-f53a3a">{{$order_info['pay_rmb_amount']}}</span></li>
                    </ul>
                </div>
                    ...
                    <ul class="flex-end">
                        ...
                        @elseif($order_info['plat_order_state']==2)
                            ...
                            @if($order_info['is_get_gift'] == 0)
                                <input type="hidden" value="{{$order_info['plat_order_id']}}"/>
                               {{-- <li class="flex-center cancel-order" is-share="{{$order_info['is_share_gifts']}}">取消订单</li>--}}
                                @if($order_info['is_share_gifts'] == 0)
                                    <a href="/order/applyRefund/{{$order_info['plat_order_id']}}"><li class="refund-order111 flex-center" order="{{$order_info['plat_order_id']}}">申请退单</li></a>
                                @endif
                            @endif
...
@section('js')
    ...
    <script type="text/javascript" src="{{asset('sd_js/order/order_list.js')}}"></script>
