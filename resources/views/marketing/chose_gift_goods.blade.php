...
@section('css')
    ...
    <link href="{{asset('css/giftcard/gifts.css')}}" rel="stylesheet">
    <link href="{{asset('css/giftcard/hexiao.css')}}" rel="stylesheet">
    ...
    <style>
        ...
        .quantity-increase em {
            background: rgba(0, 0, 0, 0) url("{{asset('img/giftcard/cart-number.png')}}") no-repeat scroll 0 0 / 100% auto;
            display: block;
            height: 10px;
            margin: 7px;
            width: 10px;
        ...
        .quantity-decrease em {
            background: rgba(0, 0, 0, 0) url("{{asset('img/giftcard/cart-number.png')}}") no-repeat scroll 0 -18px / 100% auto;
            display: block;
            height: 10px;
            margin: 7px;
            width: 10px;
...
@section('content')
    ...
    <div style="margin-bottom:40px;">
        <img class="top_img" src="{{$scanData['activity_images']}}" width="100%">
        <div style="background-color: rgb(240,240,240);height: 10px;width: 100%;"></div>
        <input type="hidden" id="mid" value="{{$scanData['member_id']}}">
        <input type="hidden" id="giftcoupon_id" value="{{$scanData['giftcoupon_id']}}">
        <input type="hidden" id="card_id" value="{{$scanData['card_id']}}">
        @foreach($scanData['packages'] as $k => $package)
            <div class="gift_card">
                <div class="gift_card_list">
                    <div class="left" event_status="false" pid="{{$package['package_id']}}">
                        @if($package['useable_num'] == 0)
                            <img class="disabled" style="width:26px;height:26px;"
                                 src="{{asset('img/address/address-default-disabled.png')}}">
                        @else
                            <img class="noselected" src="{{asset('img/address/address-default-pre.png')}}">
                            <img class="selected" src="{{asset('img/address/address-default-set.png')}}"
                                 style="display: none">
                        @endif
                    </div>
                    <div class="images">
                        <img src="{{$package['package_images']}}">
                    </div>
                    <div class="mid">
                        <p class="title">{{$package['package_name']}}</p>
                        <p class="amount">数量：{{$package['goods_num']}}种</p>
                    </div>
                    <div class="right">
                        @if($k == 0)
                            <img src="{{asset('img/giftcard/gift_down.png')}}">
                            <img src="{{asset('img/giftcard/gift_next.png')}}" style="display: none">
                        @else
                            <img src="{{asset('img/giftcard/gift_down.png')}}" style="display: none">
                            <img src="{{asset('img/giftcard/gift_next.png')}}">
                        @endif
                    </div>
                </div>
                <div class="gift_cart_detail" @if($k != 0)style="display: none"@endif>
                    @foreach($package['goodsList'] as $k1 => $goods)
                        <div style="width:9%;display: block;float:left;height:20px;"></div>
                        <div class="gift_cart_detail_list" event_status="false" pid="{{$package['package_id']}}"
                             maxnum="{{$goods['useable_num']}}" price="{{$goods['price']}}"
                             sid="{{$goods['sku_id']}}" cgid="">
                            <div class="select">
                            ...
                            <div class="left1">
                                <div class="photo" sid="{{$goods['sku_id']}}">
                                    <img src="{{$goods['sku_image']}}">
                                </div>
                            </div>
                            <div class="right">
                                <div class="title" sid="{{$goods['sku_id']}}">
                                    <p>{{$goods['sku_name']}}</p>
                                </div>
                                <div class="amount">数量：
                                    <div class="quantity-wrapper" style="float:right;">
                                        ...
                                        <input type="text" class="quantity" value="{{$goods['useable_num'] <= 0 ? 0 : 1}}" size="4"/>
                                        <a href="javascript:void(0)" class="quantity-increase quantityPlus">
                                            <em id="plus">+</em>
                                        </a>
                                        <span style="padding:5px;line-height:27px;font-size:12px;color:rgb(80,80,80)">库存：{{$goods['useable_num']}}件</span>
                                    </div>
                                </div>
                            </div>
                        </div>
    ...
    <form action="{{asset('order/showPay')}}" method="post" id="giftForm">
...
@section('js')
    ...
    <script type="text/javascript" src="{{asset('js/gifts_card.js')}}"></script>
    <script type="text/javascript">
        var arr = new Array();
        arr['ok'] = "false";
        arr['false'] = "ok";
        $(function () {
            if ('{{$scanData['member']['mobile_bind']}}' == '0') {
                $('#exampleModal1').modal({
                    show: true,
                    backdrop: 'static',
                    keyboard: false
            ...
            $('.btn').on('click', function () {
                ...
                showConfirmDialog('确认领取当前选中的商品吗？选择后礼品套餐将不可变更，套餐内剩余礼品将保存至我的礼包', function () {
                    ...
                    $.post('{{asset('personal/wallet/giftCoupons/saveCouponGoods')}}', {
