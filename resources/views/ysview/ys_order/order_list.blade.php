...
@section('content')
    ...
    <div class="mui-segmented-control mui-segmented-control-inverted mui-segmented-control-primary b-white">
        <a class="mui-control-item  font-12 @if(empty($state)) mui-active @endif" href="/ys/order/index">全部</a>
        <a class="mui-control-item font-12 color-80 @if($state == '1') mui-active @endif" href="/ys/order/index/1">待付款</a>
        <a class="mui-control-item font-12 color-80 @if($state == '2') mui-active @endif" href="/ys/order/index/2">待发货</a>
        <a class="mui-control-item font-12 color-80 @if($state == '3') mui-active @endif" href="/ys/order/index/3">待收货</a>
        <a class="mui-control-item font-12 color-80 @if($state == '9') mui-active @endif" href="/ys/order/index/9">已完成</a>
    </div>
            ...
            @foreach($orders_info as $order_info)
                <div class="order-list">
                    ...
                    @foreach($order_info['skus'] as $goods)
                        <a href="{{asset('ys/order/info/'.$order_info['plat_order_id'])}}">
                        ...
                        <ul class="flex-end">
                            ...
                            @elseif($order_info['plat_order_state']==2)
                                ...
                                @if($order_info['is_get_gift'] == 0)
                                    <input type="hidden" value="{{$order_info['plat_order_id']}}"/>
                                    {{-- <li class="flex-center cancel-order" is-share="{{$order_info['is_share_gifts']}}">取消订单</li>--}}
                                    <li class="refund-order flex-center" order="{{$order_info['plat_order_id']}}">申请退款</li>
                                @endif
