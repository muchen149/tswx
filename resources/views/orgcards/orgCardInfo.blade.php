...
@section('content')
    ...
    <div class="orgCard-top" >
        <img  src="{{$orgCard->card_image}}" alt="">
    </div>
        @if($hasFlag==1)
            <div style="width:100%;margin-top: -30px;;align-content: center;text-align:center;">
                <span  style="font: 微软雅黑;font-size: medium;">所属组织:{{$flagName}}</span>@if($orgCard->edit_flag==1)<a href="javascript:show()"> 修改 </a>@endif
            </div>
        @endif
    <div class="orgCard-top-1" >
        <div class="cloumn">
            <p>积分</p>
            <p>{{$memberOrgCard->org_points}}</p>
        </div>
        <div class="fenge" ></div>
        <div class="cloumn">
            <p>余额</p>
            <p>{{$memberOrgCard->orgcard_balance}}</p>

        </div>
        {{--<div class="fenge" ></div>
        <div class="cloumn">
            <p>水丁币</p>
            <p>{{$member->yesb_total}}</p>
        </div>--}}
    </div>

    <div class="orgCard-top-2" >
        <a href="{{asset('/index')}}"><img src="{{asset('orgcard_img/btnUseCard.png')}}"  /></a>
    </div>

    <div class="orgCard-top-3" >
        <ul class="fields" style="margin: 0 auto;display:inline;">
            {{--<a href="{{asset('/orgcards/orgcardGoods')}}"><li style="margin: 5px 0 5px 0;width:90%;border-bottom: solid; border-top: solid;border-style: ridge;font-size: 24px;text-align: left;" >卡权益 <img style="margin: 5px 0 0 5px;float:right;" src="{{asset('orgcard_img/img_right.png')}}" /></li></a>--}}

            @if($orgCard->edit_flag!=1)
                <a href="{{asset('/orgcards/orgcardGoods/'.$orgCard->id)}}"><li style="margin: 5px 0 5px 15px;width:90%;border-bottom: solid; border-top: solid;border-style: ridge;font-size: 20px;padding:3px;text-align: left;" >专属商品<img style="margin: 5px 0 0 5px;float:right;" src="{{asset('orgcard_img/img_right.png')}}" /></li></a>
            @endif
            @if($orgCard->card_detail_link!='')
            <a href="{{$orgCard->card_detail_link}}"><li style="margin: 5px 0 5px 15px;width:90%;border-bottom: solid; border-top: solid;border-style: ridge;font-size: 20px;padding:3px;text-align: left;">卡详情 <img style="margin: 5px 0 0 5px;float:right;" src="{{asset('orgcard_img/img_right.png')}}" /></li></a>
            @endif
        </ul>
    </div>
        ...
        <div>@if($hasFlag==1)
                <form id="form1" class="form" action="/orgcards/updateFlag" method="post" name="form1" >
                    <div style="width:100%;margin-top: 36px;;align-content: center;text-align:center;">
                        <input type="hidden" name="card_id" value="{{$orgCard->id}}" />
                        <input type="hidden" name="orgid" value="{{$orgCard->orgid}}" />
                    <span  style="font: 微软雅黑;font-size: medium;">所属组织
                        </span><span>
                    <select id="org_flag" name="org_flag"  required="required">
                        <option value="0">-请选择-</option>
                        @foreach($orgFlags as $orgFlag)
                            <option value="{{$orgFlag->id}}">{{$orgFlag->name}}</option>
                        @endforeach
                    </select>
                            </span>
                        {{--<input type="button" value="保存" onclick="javascript:updateFlag();"/></span>--}}
                        {{--<span><a href="javascript:javascript:updateFlag();">保存</a></span>--}}
                    </div>
                </form>
            @endif</div>
    </div>
