...
@section('css')
    ...
    <link type="text/css" rel="stylesheet" href="{{asset('form_css/orgcardview.css')}}"/>
...
@section('content')
    ...
    <div class="orgCard-top-1" >
        <p style="font: 微软雅黑;font-size: medium;font-weight: bold;">{{$orgCard->card_desc}}</p>
    </div>

    <input id="hasFlag" name="hasFlag" value="{{$hasFlag}}" type="hidden"/>
    <div class="orgCard-top-2" >
        <form id="form1" class="form" action="/orgcards/add" method="post" name="form1" >
            ...
            <div class="formData" >
                ...
                @if($form)
                <input id="FRMID" name="FRMID" autofill="" value="{{$form->fid}}" type="hidden"/>
                @endif
                <input id="card_id" name="card_id" autofill="" value="{{$orgCard->id}}" type="hidden"/>
                <input id="orgid" name="orgid" autofill="" value="{{$orgCard->orgid}}" type="hidden"/>

            <div id="btnSubmit" class="btn-Submit">
                <div><img src="{{asset('orgcard_img/btnGetOrgCard.png')}}" /></div>
            </div>
