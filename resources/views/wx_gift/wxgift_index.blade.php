...
@section('content')
    ...
    <div class="mui-segmented-control mui-segmented-control-inverted mui-segmented-control-primary" style="padding: 0 6px;background-color: #a41f24">
        @foreach($label_goods_list as $key => $label_list)
             @if($key == 0)
                <a class="mui-control-item  font-14 mui-active" style="color:#fff;" href="#item{{$label_list['share_gifts_label_id']}}">{{$label_list['share_gifts_label_name']}}</a>
             @else
                <a class="mui-control-item font-14 color-80" style="color:#fff;" href="#item{{$label_list['share_gifts_label_id']}}">{{$label_list['share_gifts_label_name']}}</a>
            @endif
        @endforeach

    </div>
    <div class="container" style="background:#a41f24;">
    <div class="s-listAll" id="product_list">
        @foreach($label_goods_list as $key => $label_list)
            <div id="item{{$label_list['share_gifts_label_id']}}" class="mui-control-content @if($key == 0) mui-active @endif ">

                <ul class="clearFix">
                @foreach($label_list['g_list'] as $k => $good_info)
                        <li>
                            <a href="{{url('/shop/goods/spuDetail/'.$good_info['spu_id'])}}" style="background: #fff;"><img src="{{$good_info['main_image']}}" width="100%" alt="">
