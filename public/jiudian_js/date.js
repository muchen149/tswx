...
var date = function ($) {
  $.fn.hotelDate = function (options) {
    ...
    function initDay() {
      ...
      var enterTime = enterMonth + '/' + enterDay;
      // 获取离开时间
      var leaveYear = String(nextDate.getFullYear());
      var leaveMonth = String(nextDate.getMonth() + 1);
      leaveMonth.length === 1 ? leaveMonth = '0' + leaveMonth : false;
      var leaveDay = String(nextDate.getDate());
      leaveDay.length === 1 ? leaveDay = '0' + leaveDay : false;
      var leaveTime = leaveMonth + '/' + leaveDay;
      $('.entertime').text(enterTime); // 显示
      $('.leavetime').text(leaveTime);
...
//  $.fn.initShowDate = initShow(ed,ld);  initShow

  $.fn.initShowDate=function (edate,ldate) {
    //debugger;
    // 获取入住时间
    ...
    /*var enterdate=new Date(Date.parse(edate.replace(/-/g,  "/")));//string2date(edate);
    var leavedate=new Date(Date.parse(ldate.replace(/-/g,  "/")));//string2date(ldate);*/
    var enterdate=new Date(Date.parse(edate.replace(/^(\d{4})(\d{2})(\d{2})$/, "$1/$2/$3")));//string2date(edate);
    var leavedate=new Date(Date.parse(ldate.replace(/^(\d{4})(\d{2})(\d{2})$/, "$1/$2/$3")));//string2date(ldate);

    var enterYear = String(enterdate.getFullYear());
    var enterMonth = String(enterdate.getMonth() + 1);
    enterMonth.length === 1 ? enterMonth = '0' + enterMonth : false;
    var enterDay = String(enterdate.getDate());
    enterDay.length === 1 ? enterDay = '0' + enterDay : false;
    var enterTime = enterMonth + '/' + enterDay;
    // 获取离开时间
    var leaveYear = String(leavedate.getFullYear());
    var leaveMonth = String(leavedate.getMonth() + 1);
    leaveMonth.length === 1 ? leaveMonth = '0' + leaveMonth : false;
    var leaveDay = String(leavedate.getDate());
    leaveDay.length === 1 ? leaveDay = '0' + leaveDay : false;
    var leaveTime = leaveMonth + '/' + leaveDay;


    var days = leavedate.getTime() - enterdate.getTime();   //时间差的毫秒数

    //------------------------------

    //计算出相差天数
    var night=Math.floor(days/(24*3600*1000));
