...
// 返回值 amount：点击选中的该支付方式需要支付的金额（若为该支付方式的最大金额，说明选中该支付方式恰好可以支付完，若小于，说明该支付方式金额用不完就可支付成功）
    ...
    $('.id_card').on('click', function () {
        $('#light').css('bottom', 0).css('transition', 'all 0.5s');
        $('.light-xiaoguo').addClass('yinying');
    });
    $('.id_card_img').on('click', function () {
        $('#light_img').css('bottom', 0).css('transition', 'all 0.5s');
        $('.light-img-xiaoguo').addClass('yinying');
    });

    //<%--============================= 点击关闭认证身份证填写页面 ===========================%>
    $('#idCard_close').on('click', function () {
        $('#light').css('bottom', '-100%').css('transition', 'all 0.5s');
        $('.light-xiaoguo').removeClass('yinying');
    });
    $('#idCard_img_close').on('click', function () {
        $('#light_img').css('bottom', '-300px').css('transition', 'all 0.5s');
        $('.light-img-xiaoguo').removeClass('yinying');
    });
    //<%--============================= 点击关闭查看实名认证通知 ===========================%>
    ...
    $('.to_examine').on('click', function () {
        var v_name = $("#v_name").val(); //获取真实姓名
        var v_idcard = $("#v_idcard").val(); //获取身份证号码
        var y_name =/^([\u4e00-\u9fa5]){2,7}$/;       //只能是中文，长度为2-7位
        if(!y_name.test(v_name)) {
            message("请正确输入姓名!");//请将“字符串类型”要换成你要验证的那个属性名称！
            return false;
        }
        ...
        $.post('/personal/bingIdCard', data, function (res) {
            if(res.code == 0){
                $('.id_card').css('display','none');
                $('.zhiyou_baoshui').css('display','flex');
                //$(".id_card").off("click");
                //$('.zf-btn').click();
                $('#light').css('bottom', '-60%').css('transition', 'all 0.5s');
                $('.light-xiaoguo').removeClass('yinying');
                message('绑定成功！',2000);
            }else if(res.code == 1){
                message(res.message);
    ...
    //<%--============================= 点击“身份证照片验证” ===========================%>
    $('.to_examine_img').on('click', function () {
        var front = $('#hid_front').val();
        var back = $('#hid_back').val();
        var data = {
            'front_img':front,
            'back_img':back
        };
        $.post('/personal/bingIdCardImg', data, function (res) {
            debugger;
            if(res.code == 0){
                $('.id_card_img').css('display','none');
                $('.heitao_qinguan').css('display','flex');
                //$(".id_card_img").off("click");
                //$('.zf-btn').click();
                $('#light_img').css('bottom', '-300px').css('transition', 'all 0.5s');
                $('.light-img-xiaoguo').removeClass('yinying');
                message('绑定成功！',2000);
