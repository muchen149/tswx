...
$(document).ready(function () {
    ...
    $('.choose-a').bind('click', function (e) {
        ...
        $.post('/elife/goods/getSku',data,function (data) {
            ...
            if(data['type'] == 1){
                ...
                $('#choose-num').text(data['sku']['sku_storage_num']);
				...
				if (($('.a-style').length + $('.img-border').length) == $('.guigeming').length) {
					if (data['sku']['sku_storage_num'] < 1) {
						$('.choose-a').removeClass('a-style');
						message('此规格库存不足');return false;
					}
				}
            }
        });
    ...
    function joinClick(){
        ...
        $.post('/elife/cart/add', $data, function (data) {
            if (data['code'] == 0) {
                message('商品已成功加入购物车！');
                return true;
