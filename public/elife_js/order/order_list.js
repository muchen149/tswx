...
$(function(){
    ...
    $('.go-to-pay').on('click', function () {
        ...
        window.location.href = '/elife/pay/gypay?plat_order_id='+order_id+'&type=pay_again';
    });

    //再次购买
    $('.buyagain').on('click', function () {
        var spu_id = $(this).val();
        window.location.href = '/elife/goods/spuDetail/' + spu_id;
    });
   ...
   //取消订单
    ...
    $('.refund-order').on('click', function () {
        ...
        if(refund_order == true){
            $.get('/elife/pay/reject?data=' + data, function (data) {
                //console.log(data);
