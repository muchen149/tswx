...
var radioIndex=0;
var checkboxIndex=0;
var msg = {numberError:'请输入数字',dateError:'请输入日期',emailError:'邮箱错误',mobileError:'手机错误'};
DEFFLD = {
...
function createRadioItemsPreview(b, f) {
	...
	var r_name=b.TYP+radioIndex;
	radioIndex++;
	//var n = 0;
	//debugger;
	//var n=b.LBL+b.TYP;
	//var n = $('.fieldInstruct').attr("id")+b.TYP;
	//var w = $('#fields').children().find(':radio').attr('id');
	//console.log(w);
    var d = false;
    ...
    $.each(b.ITMS, function (g, c) {
		...
		/* n = e.find(':radio').parent().parent().parent().attr('#id');
		console.log(n); */
		e.find(':radio').attr("name",r_name);
		e.find(':radio').attr("value",g);
...
/* 创建多选框 */
function createCheckboxItemsPreview(q,m){
	m.empty();
	var b;
	var a;
	var i = false;
	
	var r_name = q.TYP + checkboxIndex;
	checkboxIndex++;
	...
	$.each(q.ITMS, function (j, c) {
		...
		b.find(':checkbox').attr("name",r_name+j);
		checkboxs = b.find(':checkbox').attr("value",j);
                    ...
                    if (q.TYP === 'address') {
                        ...
                        if (q.TYP === 'radio') {
                            ...
                            if (q.TYP === 'checkbox') {
								...
								createCheckboxItemsPreview(q,m)
                            } else {
                                if (q.TYP === 'image') {
