...
$(document).ready(function () {
    ...
    $('.addressContent').delegate('.xuanzhong', 'click', function () {
        ...
        var dz = $("input[name='radio']:checked").parent().parent().next().find('.dz').text();
    ...
    $('.baocun').on('click', function () {
        ...
        if (name == '' || name == null) {
            ...
            $.post('/address/save', $data, function (data) {
    ...
    $('.edit-del').on('click', function () {
        ...
        if (def == 1) {
            ...
            $.get('/address/delete/' + ycid, function (data) {
                if (data['code'] == 0) {
                    ycbtn.parent().remove();
                    $('.close-edit-btn').click();
                    message('删除成功！');
    ...
    $('.zf-btn').on('click', function () {
        ...
        $.post('/order/canBuy', {skus: $skus_arr}, function (res) {
            if (res.data == 0) {
                message(res.message, 5000);
            } else {
                $('#skus').val($skus_arr.join(','));
                $('#message').val($('textarea').val());
                $('#currency').val($('.jiubis').val());
                $('#buyMethod').val($('.change-icon').next().val());
                $('#fastBuy').val($('.fastBuy').val());
                $('#buyForm').submit();
            }
        });
