...
class TransportTemplateController extends BaseController
{
    ...
    public function goodsTransportCost(Request $request)
    {
        ...
        try {
            $data = $this->getTransportCost($tpl_transport_id, $goods_number, $sendee_city_id,$sku_id);
     ...
     */
    public function getTransportCost($tpl_transport_id, $goods_number = 1, $sendee_city_id = 0,$tpl_gSkuId=0)
    {
        $grade = 10;
        $member = Auth::user();
        if ($member) {
        ...
        if (empty($is_exist_tpl)) {
            ...
            if($is_exist_tpl->isFreeDelivery==1){
                $sku_obj=GoodsSku::where('sku_id',$tpl_gSkuId)->first();
                // 根据当前登录用户级别，获取其应享受的价格，非登录情况下，显示平台价【官网零售价】
                $goods_price = $this->getSkuPrice($grade, $sku_obj);
                $price= bcmul($goods_price, $goods_number, 2);
                if($price>=$is_exist_tpl->limitNum){
                    return $transport_cost;
                }
