...
class ArticleController extends BaseController
{
    ...
    public function articleDetaile($article_id)
    {
        ...
        $article->image_url_1 = $this->getFullPictureUrl($article->image_url_1);
        $article->image_url_2 = $this->getFullPictureUrl($article->image_url_2);
        //微信jsapi
        $signPackage = session('signPackage');
        $url = "http://$_SERVER[HTTP_HOST]";
        $share_link = $url . "/article/" . $article_id;
        return view('article.articleDetaile', compact('article','signPackage','share_link'));
    }
