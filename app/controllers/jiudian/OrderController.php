...
OrderController extends BaseController
{
    ...
    public function index($state = null)
    {
        ...
        if (empty($state)) {
            $orders = Order::listsOrder()->where('order_type', 2)->get();
            ...
            $orders = Order::listsOrder()->where('plat_order_state', $state)->where('order_type', 2)->get();
        }
...
//        Order::listsOrder()->where('plat_order_state', $state)->get();
        ...
        return view('jiudianview.jiudian_order.order_list', compact('orders_info', 'state'));
    }
    ...
    private function buildInfoByOrder($order)
    {
        ...
        $extend = OrderExtend::where('plat_order_id', $order->plat_order_id)->first();
        ...
        */
        ...
        $order_info['enterdate']=$extend->enterdate;
        $order_info['leavedate']=$extend->leavedate;
        return $order_info;
    }
    ...
    public function info($order_id)
    {
        ...
        $message = $OrderExtend->order_message;

            //判断是否关注公众号
        $subscribe = $this->getSubscribe($member_id);

        return view('jiudianview.jiudian_order.order_detail', compact('order_info', 'message', 'order_address','subscribe','OrderExtend','plat_vrb_name'));
...
//        }
        ...
        return view('wx_order.jdorder_confirm', compact(
            'is_hasAddress', 'default_address', 'province_dct',
            'skus_info', 'fare_total', 'payable_amount', 'my_money_info', 'pay_rmb',
            'is_fastBuy', 'sku_source_type', 'payment_type', 'grade', 'expire_arr','nights','enterdate','leavedate'
        ));
    }
...
//            $obj_sku = GoodsSku::where('sku_id', $sku_id)->first();
    ...
    public function add(Request $request)
    {
        ...
        $enterdate=str_replace('/','',$enterdate);
        ...
        foreach ($skus as $sku) {
            ...
            $num=DB::table("sku_soldrecord")->insert($sold_rcds);
        ...
        if ($sku_source_type == 7 ||
            ...
            $address_id = 0;//(int)$request->input('address_id');
            ...
            // 收货人所在城市
            $sendee_city_id = 0;//$sendee_address->city_id;
        };
        ...
        try {
            ...
            */
            ...
            $order = $this->createOrderBySkus($member, $order_info,
                $sendee_address, $arr_payment_info, $sku_source_type, $pay_mode_id, $expire_time,2);
            if ($order) {
            ...
            if($order->order_type==2){
                $extend_data['enterdate'] = $enterdate;
                $extend_data['leavedate'] = $leavedate;
