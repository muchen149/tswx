...
use App\models\member\MemberIdCard;
...
class MemberController extends BaseController
{
     ...
     * @return View
     */
    public function index(MemberShipDao $memberShipDao)
    {
        ...
        // 我的资产 => 会员卡
        $cardList = $memberShipDao->getEnableCardByMemberId($member_id);
        $card = count($cardList);
        ...
        return view("user.wx_user", compact('member', 'state_num_arr', 'plat_vrb_caption', 'wallet_num_arr','card','goods_num_in_cart','subscribe'));
    }
...
GROUP BY member_id,member_name ) tt order by total desc';
    ...
    Public function bingIdCard(Request $request)
    {
        ...
        if($idCard && $idName){
            $id = MemberIdCard::where('member_id',$member_id)->first();
            if($id){
                //防止恶意绑定(一小时只能绑定10次)
                if($id->record_num == 10){
                    $current_time = time();
                    $diff_time = ($current_time - $id->update_time)%86400/60;
                    if($diff_time < 60){
                        $time = ceil(60-$diff_time);
                        return Api::responseMessage(3,'','操作过于频繁,请'.$time.'分钟后绑定');
                    }else{
                        $record_num =0;     //当时间大于一分钟时，将记录次数恢复为0
                    }
                }
            }

            //认证身份证号
            $idCardQuery = IdCardVerify::idCardQuery($idCard,$idName);
            if($idCardQuery['res'] == '1'){
                if($id){
                    if(isset($record_num)){
                        $MemberIdCard = MemberIdCard::where('member_id',$member_id)->update(['idcard'=>$idCard,'realname'=>$idName,'update_time'=>time(),'record_num'=>0]);
                    }else{
                        $record_num = $id->record_num;
                        $record_num++;
                        $MemberIdCard = MemberIdCard::where('member_id',$member_id)->update(['idcard'=>$idCard,'realname'=>$idName,'update_time'=>time(),'record_num'=>$record_num]);
                    ...
                    $MemberIdCard = MemberIdCard::create(['member_id'=>$member_id,'idcard'=>$idCard,'realname'=>$idName,'create_time'=>time()]);
                    if($MemberIdCard){
                        return Api::responseMessage(0);
                    }
                }
            }else{
                return Api::responseMessage(1,'','身份证号码和真实姓名不一致');
            }
        }
        return Api::responseMessage(2,'','绑定失败');
    }
     ...
     * @return mixed
     */
    public function bingIdCardImg(Request $request)
    {
        //判断当前用户是否登陆过
        ...
        $front_img = $request->input('front_img');
        $back_img = $request->input('back_img');
        if($front_img && $back_img){
            $front_img_base64 = base64_encode(file_get_contents($front_img));
            $back_img_base64 = base64_encode(file_get_contents($back_img));
            $id = MemberIdCard::where('member_id',$member_id)->first();
            //防止恶意绑定(一小时只能绑定10次)
            if($id){
                if($id->record_num == 10){
            ...
            //正面认证
            $front = IdCardVerify::imgIdCard($front_img_base64,'front');
            if($front){
                $front_data = [
                    'idcard'    => $front['idcard'],
                    'realname'  => $front['realname'],
                    'sex'       => $front['sex'],
                    'born'      => $front['born'],
                ...
                return Api::responseMessage(1,'','正面图片错误');
            }

            //反面认证
            $back = IdCardVerify::imgIdCard($back_img_base64,'back');
            if($back){
                $back_data = [
                    'begin'     => $back['begin'],
                ...
                return Api::responseMessage(1,'','反面图片错误');
            }
            $data = array_merge($front_data,$back_data);
            if($id){
                $data['update_time']=time();
                if(isset($record_num)){
                    $data['record_num'] = 0;
                    $MemberIdCard = MemberIdCard::where('member_id',$member_id)->update($data);
                }else{
                    $record_num = $id->record_num;
                    $record_num++;
                ...
                $MemberIdCard = MemberIdCard::create($data);
                if($MemberIdCard){
                    return Api::responseMessage(0);
                }
            }
    ...
    {
        if (empty($_FILES)){
            $result['message'] = '上传图片不存在';
            return json_encode($result);
        }

        //$images = isset($_FILES['front_img']) ? $_FILES['front_img'] : '';
        //上传图片
        if(isset($_FILES['front_img'])){
            if(($_FILES["front_img"]["size"] / 1024 / 1024) > 4){

                $result['message'] = '图片大小不能超过4M';
                return json_encode($result);
            }
            $images = $_FILES['front_img'];
            $rootPath = config('upload')['rootPath'];
            $path = '/idcard/' . date('Y-m-d') . '/' . rand(0, 100);
            ...
            //$file_name = '8e7465d417ef9d93c696cb75c26641c3.jpg';
            $imgPath = $imgPath . '/' . $file_name;
            //这里调用的比例压缩图片大小的函数
            if (@move_uploaded_file($images['tmp_name'], $imgPath)) {
                $compressed_image = $this->compressed_image($imgPath,$imgPath);
                if($compressed_image == ''){
                    $result['message'] = '图片类型错误';
                }else{
                    $result['front_file_name'] = $this->img_domain . $path . '/' . $file_name;
                    $result['side'] = 'front';
                }
            }else{
        ...
        if(isset($_FILES['back_img'])){
            if(($_FILES["back_img"]["size"] / 1024 / 1024) > 4){

                $result['message'] = '图片大小不能超过4M';
                return json_encode($result);
            }
            $images = $_FILES['back_img'];
            $rootPath = config('upload')['rootPath'];
            $path = '/idcard/' . date('Y-m-d') . '/' . rand(0, 100);
            $imgPath = $rootPath . $path;
            if (!file_exists($imgPath)) {
            ...
            if (@move_uploaded_file($images['tmp_name'], $imgPath)) {
                ...
                if($compressed_image == ''){
                    ...
                    $result['back_file_name'] = $this->img_domain . $path . '/' . $file_name;
                    $result['side'] = 'back';
                }
            } else {
                $result['message'] = '图片上传失败';
     ...
     * @return mixed
     */
    public function delIdImg(Request $request)
    {
    ...
    public function compressed_image($imgsrc,$imgdst)
    {
        ...
        //$new_width = ($width>300?243:$width)*0.9;
        //$new_height =($height>200?153:$height)*0.9;
        if($width>1200 || $width<900){
            $new_width = 900 * 0.9;
        }else{
            $new_width = $width * 0.9;
        }
        if($height>700 || $height<500){
            $new_height = 600 * 0.9;
        }else{
