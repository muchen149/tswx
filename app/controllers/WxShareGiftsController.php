...
class WxShareGiftsController extends BaseController
{
    ...
    public function index(){
        ...
        //微信送礼广告图
        $advertList = array('advert_a0201' => []);
        $advertList['advert_a0201'] = $this->getAdvertList("'A0201'", 4);
        return view("wx_gift.wxgift_index", compact('advertList','label_goods_list'));
    }
     ...
     */
     public function order_confirm(Request $request){
         ...
         //获取商品的sku主图及运费
         $obj_spu = GoodsSpu::select('freight','tpl_transport_id')->where('spu_id', $spu_id)->first();
         ...
         //20170907-修改为运费模板计算方式--start
         /*//if($obj_spu->freight > 0){ //不免运费,读取微信分享礼品系统设置的固定值——20170906设置微信送礼默认不免邮--start
         ...
         if ($transport_cost == 0) {
             ...
             if ($tpl_transport_id) {
                 //计算单个商品的运费，参数：模板id，1个商品默认无区域，skuid
                 $single_cost = $obj_templateController->getTransportCost($tpl_transport_id, 1, 0,$sku_id);
                 // 运费合计=单个商品运费*商品数量
                 $freight_total=bcmul($single_cost, $gift_num, 2);
         ...
         $define_skus = array();
         $data['sku_id'] = $sku_id;
         $data['number'] = $gift_num;
         $data['market_price'] = $sku_info->market_price;
         $data['price'] = $sku_info->price;
         $data['groupbuy_price'] = $sku_info->groupbuy_price;
         $data['trade_price'] = $sku_info->trade_price;
   ...
   public function giftToShare($share_gifts_info_id){
       ...
       return view('wx_gift.wxgift_share', compact('share_info','signPackage','share_link','nick_name','subscribe'));
