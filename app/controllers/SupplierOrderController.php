...
use App\controllers\thirdapi\WyKlThirdApiController;
use App\controllers\thirdapi\WyKlThirdApiControllerTwo;
...
class SupplierOrderController extends BaseController
{
    ...
    public function orderSplit($plat_order_id)
    {
        ...
        */
        ...
        //拆单成功后保存主订单信息的派单状态为已派单--20170905
        Order::where('plat_order_id', $plat_order_id)->update(array('split_state' => 1));
        // 拆单成功
            ...
            if ($obj_spu) {
                ...
                if ($transport_cost == 0.00) {
                    ...
                    if ($tpl_transport_id) {
                        ...
                        $transport_cost = $obj_templateController->getTransportCost($tpl_transport_id, $goods_number, $sendee_city_id,$supplier_sku['sku_id']);
                    }
                }
            }
    ...
    public function thirdCreateOrder($plat_order_id, $from_plat_code)//($supplier_order_id, $from_plat_code)
    {
        ...
        $sku_sql = "SELECT gsk.from_plat_skuid,og.number,og.sku_name,og.goods_price,gsk.from_plat_spuid FROM " . $db_prefix . "order_goods AS og
                    INNER JOIN " . $db_prefix . "goods_sku AS gsk ON gsk.sku_id=og.sku_id
                    WHERE og.plat_order_id=" . $order->plat_order_id . " and gsk.from_plat_code=" .$from_plat_code ;
        ...
        switch ($from_plat_code) {
            ...
            case '2003':
                $wyklThirdApiController = new WyKlThirdApiController();
                $result=$wyklThirdApiController->orderConfirm($order);
                break;

            case '2004':
                $wyklThirdApiControllerTwo = new WyKlThirdApiControllerTwo();
                $result=$wyklThirdApiControllerTwo->orderConfirm($order);
                break;
