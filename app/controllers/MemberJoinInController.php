...
//    }
   ...
   public function invite_friend(){
       ...
       $plat_signature = $this->getPlatSetting('plat_signature');


       return view('member.my_member_inviteFriend', compact('member_id','nick_name','signPackage','share_link','card_balance_total','wallet_total','yesb_total','people_num','membercard_total','plat_vrb_name','plat_signature'));
     ...
     * $member_id 分享者id
     */
    public function toGetMemberCard($activity_id){
        if($activity_id==14||$activity_id==15) {
            $member = Auth::user();
            if ($member) {
                $member_id = $member->member_id;
            } else {
            ...
            $activity = DB::table('membership_activity')
                ->where('activity_id', $activity_id)//邀请好友得会员专用活动id
                ->where('supplier_id', 0)
                ->where('use_type', 2)
                ->first();

            $ms = MemberShip::where('activity_id', $activity->activity_id)->where('member_id', $member_id)->first();
            //如果领过则不能在领取
            if (!$ms) {
