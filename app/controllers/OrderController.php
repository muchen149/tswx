...
class OrderController extends BaseController
{
    ...
    public function index($state = null)
    {
        ...
        if (empty($state)) {
            ...
            //完成订单时获得的积分
            foreach($orders as $pay_rmb){
                $pay_rmb['getIntegral'] = $this->getMoneyLimit($member->orgid,$pay_rmb->pay_rmb_amount);
            }
        } else {
            $orders = Order::listsOrder()->where('plat_order_state', $state)->get();
        }
...
//        Order::listsOrder()->where('plat_order_state', $state)->get();
        ...
        return view('wx_order.order_list', compact('orders_info', 'state','subscribe'));
    }
    ...
    public function info($order_id)
    {
        ...
        //判断该订单是否为普通订单还是微信送礼订单还是得到的微信礼品订单
        if ($order->is_share_gifts == 1) { //wx送礼订单
            //查看微信送礼记录详情
//            $db_prefix = $this->db_prefix;
//            $sql_query = "select *
//                      from " . $db_prefix . "share_gifts_info
//                      where plat_order_id = ".$order->plat_order_id;
//            $query_result = DB::select($sql_query, []);

            $share_info = DB::table('share_gifts_info')->where('plat_order_id', $order_id)->first();

            $get_info = DB::table('get_gifts_info')->where('share_gifts_info_id', $share_info->share_gifts_info_id)->get()->toArray();
            ...
            //查看该礼品被领取列表详情
            return view('wx_order.gift_order_detail', compact('order_info', 'message', 'share_info', 'get_info', 'signPackage', 'share_link','subscribe','plat_vrb_name'));

        } elseif ($order->is_get_gift == 1) { //得到wx礼品订单

            $get_info = DB::table('get_gifts_info')
                ->where('plat_order_id', $order_id)
                ->where('member_id', $member_id)
                ->where('is_share_sponsor', 0)
                ->first();

            $share_info = DB::table('share_gifts_info')->where('share_gifts_info_id', $get_info->share_gifts_info_id)->first();

            return view('wx_order.get_order_detail', compact('order_info', 'message', 'order_address', 'get_info', 'share_info','subscribe','plat_vrb_name'));
        } else {
            //普通订单
            return view('wx_order.order_detail', compact('order_info', 'message', 'order_address','subscribe','plat_vrb_name'));
...
//        }
        ...
        //验证身份证绑定
        $idCard = DB::table('member_idcard')->where('member_id',$member_id)->first();
        //$importType::0:直邮，1：保税，2：海淘 3:国内贸易 4:个人清关 10:其他商品
        if(!$idCard){
            foreach($define_skus as $spu){
                $importType[] = $spu['importType'];
            }
            $importType = array_unique($importType);

            switch($importType){
                case in_array(2,$importType):
                    $id_card = 3;
                    break;
                case in_array(4,$importType):
                    $id_card = 3;
                    break;
                case in_array(0,$importType):
                    $id_card = 2;
                    break;
                case in_array(1,$importType):
                    $id_card = 2;
                    break;
                case in_array(3,$importType):
                    $id_card = 1;
                    break;
                case in_array(10,$importType):
                    $id_card = 1;
                    break;
                default:
                    $id_card = 1;
            ...
            $idCard = DB::table('member_idcard')->where('member_id',$member_id)->where('front_img','<>','')->where('back_img','<>','')->first();
            if($idCard){
                $id_card = 1;
            }else{
                $id_card = 3;
        ...
        return view('wx_order.order_confirm', compact(
...
//            $obj_sku = GoodsSku::where('sku_id', $sku_id)->first();
            ...
            $define_skus[$key]['importType'] = $obj_spu->importType;
...
//        }
    ...
    public function applyRefund($plat_order_id)
    {
        ...
        return view('wx_order.sale_order_service', compact('order_info'));
    }
    ...
    public function saleOrder(Request $request)
    {
        ...
        try {
            ...
            $member = MemberAddress::where('member_id', $member_id)->first();
            if(count($supplier_id) > 0 && count($supplier_name) > 0){
                ...
                foreach($supplier as $key_supplier_id=>$supplier_name){
                    ...
                    ServiceOrder::create([
                        ...
                        "buy_id" => $member_id,                             //买家id
                        "buy_name" => $member->recipient_name,              //买家姓名
                        "apply_member_id" => $member_id,                    //申请者ID
    ...
    public function saleOrderState()
    {
        ...
        $service_order = DB::table('service_order')->where('buy_id',$member_id)->orderBy('apply_time','desc')->get();
        ...
        return view('wx_order.sale_order_state', compact('service_order'));
