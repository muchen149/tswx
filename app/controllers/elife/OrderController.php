...
use App\controllers\TransportTemplateController;
...
class OrderController extends BaseController
{
    ...
    public function index($state = null) {
        ...
        if (empty($state)) {
            $orders = Order::listsOrder()->where('entrance_type', $getEntranceType->entrance_type)->get();
            ...
            $orders = Order::listsOrder()
                ...
                })->where('entrance_type', $getEntranceType->entrance_type)->get();
        } else {
            $orders = Order::listsOrder()->where('plat_order_state', $state)->where('entrance_type', $getEntranceType->entrance_type)->get();
        }
        $orders_info = array();
        ...
        return view('elife.order.order_list', compact('orders_info', 'state','subscribe'));
    }
	...
	public function deliver_goods(Request $request) {
		$data = $request->input();
		//支付成功：1支付成功：2取消支付：3支付失败 4，支付构件中的取消，不确定是否支付成功或失败
		if ($data['result'] == 1) {
		$getMemberId = DB::table('order')->where('plat_order_sn',$data['out_trade_no'])->first(['member_id']);
		//存储用户信息
		Auth::loginUsingId($getMemberId->member_id);
		//跳转个人待发货列表 FIXME
        return redirect('/elife/order/index/2');
		} else if($data['result'] == 2){
			return redirect('/elife/order/index/1');
		} else if ($data['result'] == 3) {
			return Api::responseMessage(1,'','支付失败');
		} else {
			return Api::responseMessage(1,'','支付异常');
		}
	}
    /**
     * 工银E生活 分行窗口订单详情
    ...
    public function info(Request $request) {
		//e生活登录授权数据
		$getRequestInfo = $request->input("params");
		//跳转公共窗口方法
		$return = $this->publicOrderAdd($getRequestInfo);	
     ...
     * @return \Illuminate\Contracts\View\Factory|\Illuminate\View\View
     */
    public function info_one(Request $request) {
		//e生活登录授权数据
		$getRequestInfo = $request->input("params");
		//跳转公共窗口方法
		$return = $this->publicOrderAdd($getRequestInfo);	
     ...
     * @return \Illuminate\Contracts\View\Factory|\Illuminate\View\View
     */
    public function info_two(Request $request) {
		//e生活登录授权数据
		$getRequestInfo = $request->input("params");
		//跳转公共窗口方法
		$return = $this->publicOrderAdd($getRequestInfo);	
	...
	/**
	 * 公共过滤方法
	 * @param  $getRequestInfo(过滤数据)
	 * @time   2018-06-13
	 * @author MUCHEN
	 */
	public function publicOrderAdd($getRequestInfo = '') {
		if (!$getRequestInfo) {
        ...
        $order = Order::selectInfoZd()
            ->where('plat_order_sn',$data['out_trade_no'])
            ->first();
		...
		$type = 'e_bank';
        return view('elife.order.order_detail', compact('order_info', 'message', 'order_address','subscribe','OrderExtend','type'));
	}
	/**
     * 订单详情
     * @param $order_sn
     * @return \Illuminate\Contracts\View\Factory|\Illuminate\View\View
     */
    public function e_info($plat_order_sn = '') {
        // 获取当前登录用户信息,用户合法性由本控制器构造函数验证
        $member_id = $this->getLoginUserId();

        // 获取订单对象
        $order = Order::selectInfoZd()
            ->where('plat_order_sn', $plat_order_sn)
            ->first();
		//根据来源指向where条件
		if (!$order) {
			$order = Order::selectInfoZd()
            ->where('plat_order_id', $plat_order_sn)
            ->first();
		}
        if (empty($order)) {
            // 60101 => '订单id不存在'
        ...
        return view('elife.order.e_order_detail', compact('order_info', 'message', 'order_address','subscribe','OrderExtend'));
    ...
    public function showPay(Request $request)
    {
        ...
        */
        ...
        switch ($sku_source_type) {
            case 1:
                // SKU来源于购物车；
                $define_skus = $this->getSkusFromMyCart($cartIds, $member->member_id);
                break;
            case 8:
...
//        }
		...
		$getStorageNum = DB::table('goods_sku')->where('sku_id',$define_sku['sku_id'])->first(['sku_storage_num']);
		if (($getStorageNum->sku_storage_num - 3) < ((int)($request->input('number')))) {
               return view('errors.stockError');
		}
...
//        }
        //实名认证须知
        //$shopNote = DB::table('document')->where('doc_code','klsmrz')->first();
        // 是否立即购买（is_fastBuy）、SKU来源类型（sku_source_type）

        //验证身份证绑定
        /*if($member->ID_card && $member->ID_name){
            $id_card = 1;
        }else{
            $importType = $request->input('importType');
            if($importType==3 || $importType ==10){
                $id_card = 1;
            }else{
                $id_card = 0;
            }
...
//        }
        ...
        try {
            ...
            */
            ...
            $order = $this->createOrderBySkus($member, $order_info,
                $sendee_address, $arr_payment_info, $sku_source_type, $pay_mode_id, $expire_time);
            if ($order) {
            ...
            $extend_data = array(
                ...
                'org_card_id' => empty($arr_payment_info['org_pay_card']['card_id']) ? 0 : $arr_payment_info['org_pay_card']['card_id'],
            );
    ...
    public function prePay(Request $request)
    {
        ...
        if (!in_array($buyMethod, $able_payMethod)) {
            ...
            return view('errors.e_error');                // 数据有误
        }
	...
	/**
	 * 确认收货 暂未调取第三方确认 严选 考拉
	 * @param $plat_order_id
	 * @author muchen
	 * @time2018-07-2
	 */
	public function transferConfirmReceipt($plat_order_id = '',$member_id = '',$type = '') {
		$member = $this->loseEfficacy();
		$plat_order = Order::where('plat_order_id', $plat_order_id)->first();
		//只操作超过15天订单
		if ($type == 'system') {
			if (($plat_order->create_time + 1296000) > time()) {
				return false;
			}
		}
		DB::table('order')->where('plat_order_id',$plat_order_id)->update(['plat_order_state'=>4,'arrival_time'=>time()]);
		//记录平台操作日志
		...
		foreach ($supplier_orders as $supplier_order) {
			...
			LogInfoFacade::logOrderSupplier($supplier_order->supplier_id, $supplier_order->supplier_order_id,$so_content, 4, $member->member_id, $member->nick_name);
		}
		//修改仓点发货表状态
    ...
    public function confirmReceipt($plat_order_id)
    {
        ...
        DB::transaction(function () use ($plat_order_id, $member) {
            ...
            */
            ...
            //机构下的买家领取积分
            $integral = $this->getMoneyLimit($plat_order->pay_rmb_amount);
            $orderExtend = OrderExtend::where('plat_order_id', $plat_order_id)->first(['org_card_id']);

            if($integral != 0){
                $points_total = $member->points_total + intval($integral);
                $member->update(['points_total'=>$points_total]);
                if($orderExtend->org_card_id != 0){
                    DB::table('member_orgcards')->where('orgcard_id',$orderExtend->org_card_id)->where('member_id',$member->member_id)->update(['org_points'=>$integral]);
                }
            }

            // 记录平台操作日志
            $content = '买家 "' . $member->nick_name . '" 已收货! 订单编号为:' . $plat_order->plat_order_sn.';'.'获得的积分为:'.$integral;
            LogInfoFacade::logOrderPlat($plat_order_id, $content, 4, $member->member_id, $member->nick_name);
    ...
    private function buildInfoByOrder($order)
    {
        ...
        */
        ...
        $order_info = array(
            ...
            'getIntegral' => $order->getIntegral,                               //订单完成增送的积分
        );
        //若不空，说明有上传的图片，则加上路径
        if ($order->pay_cert) {
...
//            $obj_sku = GoodsSku::where('sku_id', $sku_id)->first();
            ...
            if ($define_sku['promotions_type'] == 99) {
                ...
                $market_price = $this->getSkuPrice(10,$obj_sku);    //市场价格
                $price = $this->getSkuPrice(20,$obj_sku);           //零售价格
                $groupbuy_price = $this->getSkuPrice(30,$obj_sku);  //团购价格
                $trade_price = $this->getSkuPrice(40,$obj_sku);     //批发价格
            }
                ...
                if ($transport_cost == 0) {
                    ...
                    if ($tpl_transport_id) {
                        ...
                        */
                        ...
                        if(!array_key_exists($tpl_transport_id,$arry_transSkuId)){
                            $arry_transSkuId[$tpl_transport_id]=$sku_id;
                        }
            ...
            $define_skus[$key]['market_price']   = $market_price;
            $define_skus[$key]['price']          = $price;
            $define_skus[$key]['groupbuy_price'] = $groupbuy_price;
            $define_skus[$key]['trade_price']    = $trade_price;
            // 虚拟币限额
        ...
        if(!empty($arry_transCost)){
            foreach($arry_transCost as $transKey=>$transGnum){
                $transport_cost = $obj_templateController->getTransportCost($transKey, $transGnum, $sendee_city_id,$arry_transSkuId[$transKey]);
                // 运费合计
                $transport_cost_totals = bcadd($transport_cost_totals, $transport_cost, 2);
            }
        }
    ...
    private function getSkuInfo($define_skus)
    {
        ...
        foreach ($define_skus as $define_sku) {
            ...
            $arr = [
                ...
                'number' => $define_sku['number'],
                'market_price' => $define_sku['market_price'],
    ...
    private function createOrderBySkus($member, $order_info, $sendee_address,
                                       $arr_payment_info = [], $sku_source_type = 0, $pay_mode_id = 1, $expire_time = 0)
    {
		$member = $this->loseEfficacy();
        ...
        */
        ...
        if ($payable_amount > 0) {
            $pay_rmb_amount = $this->getPayRmbAmount($payable_amount, $pay_vrb_amount,
                $pay_wallet_amount, $pay_org_amount,$arr_payment_info);
        }
        ...
        $data = [
            ...
            'pay_org_amount' => $pay_org_amount,                                        //机构卡抵扣额度
			...
			'pay_mode_code'=>'e_pay',										   //支付代码
			'pay_mode_name'=>'工银E支付',									   //支付方式
			'entrance_type'=>$getEntranceType->entrance_type,								   //支付窗口类型
...
//        }
     ...
     *
     */
    public function applyRefund($plat_order_id)
    {
        // 获取当前登录用户信息(验证)
        $this->getLoginUserId();
        // =================================== 验证数据合法性 =================================
        ...
        if($plat_order->is_share_gifts == 1){
            ...
            if($rest_share_num == 0){
                $errorData = array('code' => 10000, 'message' => '礼品已被领取完毕!');
                return view("errors.e_error", compact('errorData'));
            }
            $order = Order::listsOrder()->where('plat_order_id', $plat_order_id)->first();
            $order_info = $this->buildInfoByOrder($order);
                ...
                if (count($arr_payment_info) > 0) {
                    foreach ($arr_payment_info as & $row) {
                        ...
                        switch ($pay_type) {
                            ...
                            case "vrb":
                                if($plat_order->is_share_gifts == 1){
                                    $order_info['payment']['pay_vrb']['pay_amount'] = (int)($row['pay_amount']/$share_gifts_info->gifts_num)*(int)$rest_share_num;
                                }else{
                                    $order_info['payment']['pay_vrb']['pay_amount'] = $row['pay_amount'];
                                }
                                break;
                            case "wallet":
                                ...
                                $busine_type = 'DDTDTK';
                                if($plat_order->is_share_gifts == 1){
                                    $order_info['payment']['pay_wallet']['pay_amount'] = bcsub($row['pay_amount']/(int)$share_gifts_info->gifts_num*(int)$rest_share_num,0.00,2);
                                }else{
                                    $order_info['payment']['pay_wallet']['pay_amount'] = $row['pay_amount'];
                                }
                                break;
                            case "card_balance":
                                $card_lst = (array)$row['card_lst'];
                                if (count($card_lst) > 0) {
                                    ...
                                    $order_info['payment']['pay_card_balance']['pay_amount'] = 0.00;
                                    foreach ($card_lst as $card) {
                                        if($plat_order->is_share_gifts == 1){
                                            $balance_pay_amount += $card['balance_pay_amount']/(int)$share_gifts_info->gifts_num*(int)$rest_share_num;
                                            $order_info['payment']['pay_card_balance']['pay_amount'] = bcsub($balance_pay_amount,0.00,2);
                                        }else{
                                            $order_info['payment']['pay_card_balance']['pay_amount'] += $card['balance_pay_amount'];
                                        }
                                    }
                                }
                                break;
            ...
            //$goods = DB::table('store_deliver_goods')->where('plat_order_id',$plat_order_id)->orderBy('supplier_id')->get();

            $order = Order::listsOrder()->where('plat_order_id', $plat_order_id)->first();
            $order_info = $this->buildInfoByOrder($order);
            foreach($order_info['skus'] as &$sku){
                $db_prefix = config('database')['connections']['mysql']['prefix'];
                $spbsel= "SELECT spb.supplier_name,spb.supplier_id FROM  ".$db_prefix."supplier_baseinfo spb where spb.supplier_id in(
                 SELECT  spsku.supplier_id from ".$db_prefix."supplier_goods_sku spsku where sku_id = ".$sku['sku_id'].")";
                $spb_name=DB::select($spbsel);
                $sku['supplier_name'] = $spb_name[0]->supplier_name;
                $sku['supplier_id'] = $spb_name[0]->supplier_id;
            }
            //对供应商排序
            usort($order_info['skus'], function($a, $b) {
                $al = $a['supplier_id'];
                $bl = $b['supplier_id'];
                if ($al == $bl)
                    return 0;
        ...
        return view('elife.order.sale_order_service', compact('order_info'));
    }
    ...
    public function saleOrder(Request $request) {
        ...
        // 当前登录用户信息
        $memberInfo = $this->loseEfficacy();
        $supplier_id = $request->input('supplier_id');
        $supplier_name = $request->input('supplier_name');
        $plat_order_id = $request->input('plat_order_id');
        $sku_ids = $request->input('sku_ids');
        $arr_spskus=array();    //处理前台传过来的sku
        foreach($sku_ids as $val){
            $tmp_arr=explode('_',$val);
            if(array_key_exists($tmp_arr[0],$arr_spskus)){
                array_push($arr_spskus[$tmp_arr[0]],$tmp_arr[1]);
            }else{
                $arr_spskus[$tmp_arr[0]]=[$tmp_arr[1]];
            }
        }
        $order = DB::table('order')->where('plat_order_id',$plat_order_id)->first();

        try {
            DB::beginTransaction();

            $member = MemberAddress::where('member_id', $memberInfo->member_id)->first();
            if(count($supplier_id) > 0 && count($supplier_name) > 0){
                $supplier = array_combine($supplier_id,$supplier_name);

                foreach($supplier as $key_supplier_id=>$supplier_name){
                    //判断是否已经申请退单,避免再次添加
                    $Service_data = ServiceOrder::where('supplier_id',$key_supplier_id)->where('plat_order_id',$plat_order_id)->first();
                    if($Service_data){
                        return Api::responseMessage(1);
                    }
                    $define_skus=array();       //整合sku数组
                    $settlement_price =0.00;    //平台与供应商结算价格
                    foreach($arr_spskus[$key_supplier_id] as $sku_id){
                        $data = OrderGoods::where('sku_id',$sku_id)
                            ->where('plat_order_id',$plat_order_id)->first();
                        $sku = $data->toArray();
                        $new_sku = array_merge($sku,['price'=>0]);
                        array_push($define_skus,$new_sku);

                        $settlement_price += bcsub($sku['settlement_price'] * $sku['number'],0.00,2);
                        $data->goods_state = 1;
                        $data->save();
                    }
					//更改退单状态
					DB::table('order')->where('plat_order_id',$plat_order_id)->update(array('plat_order_state'=>'-3'));
                    //供应商运费
                    $total_info = $this->fillOrderGoodsInfo($define_skus, $order->sendee_city_id, $is_virtual=0);
                    // 运费应付金额 =  (运费结算金额 - 运费优惠结算金额)
                    $fare_total = bcsub($total_info['transport_cost_totals'], $total_info['transport_preferential'], 2);
                    //将sku序列化
                    $sku_id= serialize($arr_spskus[$key_supplier_id]);
                    //$y=implode(',',$arr_spskus[$supplier_id]);

					//判断窗口类型
					$getEntranceType = DB::table('member')->where('cust_id',$memberInfo->cust_id)->first();
                    //生成记录
                    ServiceOrder::create([
                        "plat_order_id" => $plat_order_id,                    //平台订单id
                        "sku_id" => $sku_id,                                  // skuid数组,将其序列化存储
                        "supplier_id"=>$key_supplier_id,                      //供应商id
                        "supplier_name"=>$supplier_name,                      //供应商name
                        "price"=> bcsub($settlement_price,0.00,2),            //商品结算价(乘以数量后的结算价格)
                        "transport_cost_totals"=>$fare_total,                 //供应商的运费

                        "service_content" => $request->input('text'),       //服务内容
                        "buy_mobile" => $member->mobile,                    //手机号
                        "service_state" => 20,                              //服务状态  目前就只用者2个 20,90
                        "apply_time" => time(),                             //申请时间
                        "buy_id" => $memberInfo->member_id,                             //买家id
                        "buy_name" => $member->recipient_name,              //买家姓名
                        "apply_member_id" => $memberInfo->member_id,                    //申请者ID
                        "apply_member_name" => $member->nick_name,           //申请者姓名
                        "entrance_type" => $getEntranceType->entrance_type          //申请者姓名
                    ]);
                }
       ...
       /* foreach($supplier_id as $sup_id){
            if($sup_id == 2){
                $data = WyYxThirdApi::cancelOrder($order->plat_order_sn);            //申请订单取消（网易）
                if($data['code'] = 200){
                    $result = \GuzzleHttp\json_decode($data['result'],true);
                    DB::table('service_order')->where('plat_order_id',$plat_order_id)->where('supplier_id',$sup_id)
                        ->update(['supplier_state'=>$result['cancelStatus'],'supplier_reason'=>$result['rejectReason']]);
                }else{
                    Log::notice('取消网易订单失败,订单id' . $plat_order_id);
                }
                Log::notice('--------refound order---success---');
                Log::alert($data);
            }
        }*/
        //return Api::responseMessage(0);
     ...
     *
     */
    public function saleOrderState($plat_order_id = null) {
        // 获取当前登录用户信息(验证)
		$member = $this->loseEfficacy();
		//判断窗口类型
		$getEntranceType = DB::table('member')->where('cust_id',$member->cust_id)->first();
        if(empty($plat_order_id)){
            $service_order = DB::table('service_order')->where('buy_id',$member->member_id)->where('entrance_type',$getEntranceType->entrance_type)->orderBy('apply_time','desc')->get();
        }else{
            $service_order = DB::table('service_order')->where('buy_id',$member->member_id)->where('entrance_type',$getEntranceType->entrance_type)->where('plat_order_id',$plat_order_id)->get();
        }
        foreach($service_order as $order_info){
            $order_info->sku_id = unserialize($order_info->sku_id);
            foreach($order_info->sku_id as $key=>$sku_id){
                $order_goods = DB::table('order_goods')->where('sku_id',$sku_id)->where('plat_order_id',$order_info->plat_order_id)->first();
                //$sku_id['sku_id_info'] = $order_goods;
                $order_info->sku_id[$key] = $order_goods;
            }
        }

        return view('elife.order.sale_order_state', compact('service_order'));
    ...
    public function logistics($id) {
        ...
        if (count($result) == 0) {
            ...
            if ($delGoods != null) {
                ...
                //刚下单的商品
                $result = null;
                return view('elife/order/logistics',compact('type'))->withResult($result);
            }
    ...
    public function iniMyMoneyInfo($payable_amount = 0.00, $goods_points_totals = 0, $goods_sku_lst = [])
    {
        ...
        */
        ...
        //5,机构卡(折扣)[0，默认指定义折扣。1，零售价【体验价格】2，团购价【小水价格】3，批发价格【老丁价格】]
        $goods_price = 0.00;
        $market_price = 0.00;
        $price = 0.00;
        $groupbuy_price = 0.00;
        $trade_price = 0.00;
        foreach($goods_sku_lst as $val){
            $market_price += bcadd($val['market_price']*$val['number'],$goods_price,'2');
            $price += bcadd($val['price']*$val['number'],$goods_price,'2');
            $groupbuy_price += bcadd($val['groupbuy_price']*$val['number'],$goods_price,'2');
            $trade_price += bcadd($val['trade_price']*$val['number'],$goods_price,'2');
        }
        $sql = 'select b.discount_type,b.id,b.card_name,b.card_count,b.card_image
                from ' .$db_prefix.'member_orgcards as a inner join '
            .$db_prefix. 'org_cards as b on a.orgcard_id = b.id and b.state= 1 and a.member_id ='.$member_id;
        $org_card_lst = DB::select($sql);
        foreach($org_card_lst as $key=>$val){

            if($val->discount_type != 0){
                $org_card_lst[$key]->card_count = 10;
                if($val->discount_type == 1){
                    $org_card_lst[$key]->price = $price;
                }
                if($val->discount_type == 2){
                    $org_card_lst[$key]->price = $groupbuy_price;
                }
                if($val->discount_type == 3){
                    $org_card_lst[$key]->price = $trade_price;
                }
            }else{
                $org_card_lst[$key]->price = $market_price;
            }
        ...
        /*if(discount_type == 0){
            foreach($goods_sku_lst as $val){
                $goods_price['market_price'] += bcadd($val['market_price']*$val['number'],$price,'2');
            }
        }elseif($val->discount_type == 1){
            foreach($goods_sku_lst as $val){
                $goods_price['price'] += bcadd($val['price']*$val['number'],$price,'2');
            }
        }elseif($val->discount_type == 2){
            foreach($goods_sku_lst as $val){
                $goods_price['groupbuy_price'] += bcadd($val['groupbuy_price']*$val['number'],$price,'2');
            }
        }elseif($val->discount_type == 3){
            foreach($goods_sku_lst as $val){
                $goods_price['trade_price'] += bcadd($val['trade_price']*$val['number'],$price,'2');
            }

        }*/

        if(count($org_card_lst) > 0 ){
            $org_count = count($org_card_lst);
            $arr_money_info['org_card_balance'] = array(
                'pay_type'      => 'org_card',
                'org_count'     => $org_count,
                'org_card_lst'  => $org_card_lst,
                'url'           => $this->img_domain.'/',
            );
        }
    ...
    private function readPaymentInfo(Request $request, $payable_amount = 0)
    {
        ...
        */
        ...
        if($request->input('orgCard_lst')){
            // 折扣抵消金额及卡详情
            $data = $request->input('orgCard_lst');
            foreach($data as $orgCard){
                $pay_orgCard_balance_amount = bcadd($orgCard['org_pay_amount'], 0.00, 2);
            }
            if (count($data) == 0) {
                Log::error('传入的折扣卡列表为空数组 控制器:OrderController@readPaymentInfo');
                return view('errors.e_error');                // 数据有误
            }
            $arr_payment_info['org_pay_card'] = array(
                'pay_type' => 'org_card_balance',
                'pay_id' => 0,
                'pay_amount' => $pay_orgCard_balance_amount,
                'card_id' => $data[0]['orgCard_id'],
            );

            $sum_pay_amount += $pay_orgCard_balance_amount;
        }
     ...
     * @param $pay_rmb_amount      decimal(10,2)     拟支付人民币金额
     */
    private function getPayRmbAmount($payable_amount = 0.00, & $pay_vrb_amount = 0, & $pay_wallet_amount = 0, & $pay_org_amount = 0,
        ...
        */
        ...
        if (count($arr_payment_info) > 0) {
            ...
            foreach ($arr_payment_info as $key => $row) {
                ...
                if ($key == 'pay_vrb') {
                    $pay_vrb_amount = $row['pay_amount'];
                } elseif ($key == 'org_pay_card'){
                    $pay_org_amount = $row['pay_amount'];
                }else{
                    if ($key != 'pay_rmb' || $key != 'org_pay_card') {
                        $pay_wallet_amount += $row['pay_amount'];
                    };
                }
            }
            ...
            // 非人民币支付总额(零钱，水币，机构卡抵扣)
            $pay_not_rmb_amount = bcadd($pay_not_rmb_amount,$pay_org_amount,2);
            // 人民币拟支付金额,不能为负数
...
//  有关微信分享礼品生成的订单
        ...
        try {
            ...
            */
            ...
            $data = [
                ...
                'pay_rmb_amount' => $pay_rmb_amount,                                         // 人民币支付额
                'pay_wallet_amount' => $pay_wallet_amount,                                  // 其它非人民币支付额度
                'pay_org_amount'=>$pay_org_amount,                                           //机构卡抵扣金额
                'payment' => $payment,                                                       // 支付信息数组序列化
	...
	/**
	 * @每分钟查询修改E生活超过20分钟待付款订单 1=>-1
	 * @time2018-06-28
	 * @member_id=152||1601 测试||正式系统服务器管理员 FIXME
	 */
	public function remove_order() {
		$getMemberInfo = DB::table('member')->where('member_id',152)->where('member_name','测试系统服务器管理员')->select(['member_id','member_name'])->first();
		if (!$getMemberInfo) {
			$getMemberInfo = DB::table('member')->where('member_id',1601)->where('member_name','正式系统服务器管理员')->select(['member_id','member_name'])->first();
		}
		$order = DB::table('order')
            ->where('pay_mode_code','e_pay')
			->where('plat_order_state',1)
			->where('delete_state',0)
			->where('lock_state',0)
			->select('plat_order_id','create_time')
			->get();
			$order = json_decode($order,true);
			if (!empty($order)) {
			foreach($order as $v) {
				$plat_order_id = $v['plat_order_id'];
				$create_time   = $v['create_time'];
				if (($create_time + 1200) < time()) {
					DB::table('order')
					->where('pay_mode_code','e_pay')
					->where('plat_order_state',1)
					->where('delete_state',0)
					...
					->update(['plat_order_state'=>'-1']);
					//添加撤单记录
					LogInfoFacade::logOrderPlat($plat_order_id,'自动撤单',-1,$getMemberInfo->member_id,$getMemberInfo->member_name);
				}
			}
			echo 'OK';
		}
	...
	/**
	 * @每天凌晨1点查询15天未确认收货订单 3=>4
	 * @time2018-07-2
	 * @author muchen
	 */
	public function automatic_receipt() {
		$getOrderInfo = DB::table('order')
		->where('plat_order_state',3)
		->where('delete_state',0)
		->where('pay_mode_code','e_pay')
		->get(['plat_order_id','member_id']);
		$getOrderInfo = json_decode($getOrderInfo,true);
		if (!empty($getOrderInfo[0]['member_id'])) {
			foreach($getOrderInfo as $v) {
				$orderId = $v['plat_order_id'];
				$memberId = $v['member_id'];		
			$this->transferConfirmReceipt($orderId,$memberId,'system');
			}
		}
	}
}