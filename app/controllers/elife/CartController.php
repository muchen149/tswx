...
class CartController extends BaseController
{
    ...
    public function index($num = 100)
    {
        ...
        $sql_query = "select a.cart_id,a.sku_id,a.number,
                    ...
                    where a.member_id = " . $member->member_id . " and a.entrance_type = ".$getEntranceType->entrance_type."
...
//            }
        ...
        return view('elife.cart.cart_list', compact('skus', 'is_showExpire'));
    }
...
//            }
//        }
		//对购物车进行整体判断 1库存不足 2已超库存 3已超库存(含已加购件数)
		$getCart = DB::table('member_cart')->where('member_id',$member->member_id)->where('sku_id',$sku_id)->where('entrance_type',$member->entrance_type)->first(['number']);
		$getStorageNum = DB::table('goods_sku')->where('sku_id',$sku_id)->first(['sku_storage_num']);
		if ($getStorageNum->sku_storage_num < 3) {
			return Api::responseMessage(1);
		}
		if (($getStorageNum->sku_storage_num - 3) < (!empty($getCart->number) ? ($num + $getCart->number) : $num)) {
			if (!empty($getCart->number)) {
				return Api::responseMessage(3);
			}
			return Api::responseMessage(2);
		}
        // 加入购物车
		...
		$insert_info = array(
				...
				'entrance_type'=>$getEntranceType->entrance_type);
        if (!$cart_sku) {
			DB::table('member_cart')->insert($insert_info);
        } else {
			if (!empty($getEntranceType->entrance_type) && $cart_sku->entrance_type <> $getEntranceType->entrance_type) {
				DB::table('member_cart')->insert($insert_info);
			} else {
            // 购物车存在该商品
            $cart_sku->number += $num;
