...
class BaseController extends Controller
{
    ...
    /**
     * 根据商品支付金额定义积分规则，获取该商品可支付的人民币额度，返回整数
     * 传入参数
     * @param  $orgid   int     积分定义获取规则【默认0:当前下单的人无所属机构;】
     * @param  $pay_rmb_amount    decimal(10,2)   商品实际支付价格【人民币】
     * @param  $plat_money_rate int     虚拟币兑换人民币汇率，即：1元人民币等于多少积分
     * 传出参数
     * @param   $points         int
     */
    function getMoneyLimit($pay_rmb_amount = 0.00)
    {
        $integral = 0; //积分
        if($pay_rmb_amount != 0.00){
            $pay_money_rate = intval($this->getPlatSetting('pay_money_rate'));
            if ($pay_money_rate <= 0) {
                $pay_money_rate = 1;
            }
            //实际支付人民币兑换成积分的额度
            $integral = floor($pay_rmb_amount * $pay_money_rate);
        }
